		...
		<article className="bg-white p-4 rounded-xl shadow-lg border border-gray-100">
			<div className="flex gap-4 items-start">
				...
				<div className="flex-1">
					...
					<div className="mt-3 flex gap-2 flex-wrap">
						{(post.tags || []).map((tag, idx) => (
							<span key={idx} className="text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded">{typeof tag === 'string' ? tag : (tag.en || tag)}</span>
						))}
					</div>
				</div>
				...
				<div className="w-36 h-28 flex-shrink-0 relative">
					{(post.image) ? (
						(() => {
							const imgSrc = (post.image || "").startsWith('http') ? post.image : `${API.defaults.baseURL}${post.image}`;
							return <img src={imgSrc} alt="" className="w-full h-full object-cover rounded-md" />;
						})()
					) : (
